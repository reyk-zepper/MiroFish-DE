...
def get_locale() -> str:
    if has_request_context():
        ...
        return locale if locale in _translations else 'de'
...
def t(key: str, **kwargs) -> str:
    ...
    if value is None:
        value = _translations.get('de', _translations.get('en', _translations.get('zh', {})))
        for part in key.split('.'):
            if isinstance(value, dict):
                value = value.get(part)
            else:
...
def get_language_instruction() -> str:
    locale = get_locale()
    lang_config = _languages.get(locale, _languages.get('de', _languages.get('en', {})))
