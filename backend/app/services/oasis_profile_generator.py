...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        # Graph provider for enriched context retrieval
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        try:
            search_hits = self.graph_provider.search(self.graph_id, comprehensive_query, limit=30)
            facts = []
            summaries = []
            for hit in search_hits:
                if hit.get("fact"):
                    facts.append(hit["fact"])
                if hit.get("summary"):
                    summaries.append(f"[{hit.get('name', entity_name)}]: {hit['summary']}")
                elif hit.get("name"):
                    summaries.append(str(hit["name"]))
            results["facts"] = facts[:20]
            results["node_summaries"] = summaries[:10]
            parts = []
            if results["facts"]:
                parts.append("相关事实:\n" + "\n".join(f"- {fact}" for fact in results["facts"][:10]))
            if results["node_summaries"]:
                parts.append("相关实体:\n" + "\n".join(f"- {summary}" for summary in results["node_summaries"][:8]))
            results["context"] = "\n\n".join(parts)
        except Exception as e:
            logger.debug(f"Graph context retrieval failed: {e}")
        return results
